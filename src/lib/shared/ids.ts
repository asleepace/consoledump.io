...
export function assertSessionId(sessionId: string | String | SessionId): SessionId {
  if (!isSessionId(sessionId)) throw new ApiError('Invalid sessionId.', { sessionId })
  return sessionId as SessionId
}

export function assertClientId(clientId: string | String | ClientId): ClientId {
  if (!isClientId(clientId)) throw new ApiError('Invalid clientId.', { clientId })
